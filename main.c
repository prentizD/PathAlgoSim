...
#define PATHDRAWSPEED 1       // TARGET_FPS * PATHDRAWSPEED = drawn tiles (that are in a Queue) per seconds
#define WEIGHTS 1
...
void draw_buttons(){
    ...
    DrawText("Stop and\nReset", button_reset.x, button_reset.y, FONTSIZE, BLACK);

    DrawRectangleRec(algo_button_background, LIGHTGRAY);                                                // Background for algorithm buttons
    DrawText("ALGORITHMS", algo_button_background.x+20, algo_button_background.y, 40, DARKGREEN);

    DrawRectangleRec(button_draw_instant_toggle, toggle_color);                                         // Toggle search path animation
    DrawText("Path animation", button_draw_instant_toggle.x, button_draw_instant_toggle.y, FONTSIZE, BLACK);

    DrawRectangleRec(button_dijkstra, YELLOW);                                                          // Start Dijkstra 
    DrawText("Dijkstra", button_dijkstra.x, button_dijkstra.y, FONTSIZE + 10, BLACK);

    DrawRectangleRec(button_aStar, GREEN);                                                              // Start A Star 
    DrawText("AStar", button_aStar.x, button_aStar.y, FONTSIZE + 10, BLACK);            

    DrawRectangleRec(button_bfs, MAROON);                                                               // Start BFS
    DrawText("Breadth\nFirst Search", button_bfs.x, button_bfs.y, FONTSIZE, BLACK);

    DrawRectangleRec(button_dfs, GRAY);                                                                 // Start DFS
    DrawText("Depth\nFirst Search", button_dfs.x, button_dfs.y, FONTSIZE, BLACK);                

    DrawRectangleRec(button_size_plus_ten, GRAY);                                                       // increase rows and colums by 10
    DrawText("Rows/Colums +10", button_size_plus_ten.x, button_size_plus_ten.y, FONTSIZE, BLACK);

    DrawRectangleRec(button_size_minus_ten, GRAY);                                                      // decrease rows and colums by 10
    DrawText("Rows/Colums -10", button_size_minus_ten.x, button_size_minus_ten.y, FONTSIZE, BLACK);    

    DrawRectangleRec(mazebutton_background, BEIGE);                                                     // background for maze buttons
    DrawText("Predefined mazes", mazebutton_background.x+20, mazebutton_background.y, 25, BLACK);    

    DrawRectangleRec(button_N_maze, GRAY);                                                              // draw N shaped maze
    DrawText("N-Maze", button_N_maze.x, button_N_maze.y, FONTSIZE, BLACK);                

    DrawRectangleRec(button_wall, GRAY);                                                                // draw N shaped maze
    DrawText("Middle wall", button_wall.x, button_wall.y, FONTSIZE, BLACK);
}
